...
const Item = ({ product }) => {
    ...
    return (
        <div className='overflow-hidden p-5 bg-white'>
            <div onClick={() => { navigate(`/collection/${product.category.toLocaleLowerCase()}/${product._id}`); scrollTo(0, 0) }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className='flexCenter bg-[#f5f5f5] overflow-hidden relative'>
            ...
            <div className='pt-3'>
                ...
                <div className='flexBetween pt-2 gap-2'>
                    ...
                    <button onClick={()=>addToCart(product._id)} className='btn-outline !py-2 !px-0 w-full !text-xs'>Add to Cart | Rs.{product.offerPrice}</button>
                </div>
            </div>
        </div>
    )
