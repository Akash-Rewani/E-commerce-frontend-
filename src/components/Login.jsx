        ...
        <div onClick={() => setShowUserLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-40 flex items-center text-sm text-gray-600 bg-black/50'>
            <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] rounded-lg shadow-xl border border-gray-200 bg-white'>
                <h3 className='bold-28 mx-auto mb-3'>
                    <span className='text-secondary capitalize mx-1'>User</span>
                    <span className='capitalize'>{state === 'login' ? 'Login' : 'Register'}</span>
                </h3>
                {state === 'register' && (
                    <div className='w-full'>
                        <p className='bold-14 '>Name</p>
                        <input type="text" onChange={(e) => setName(e.target.value)} value={name} className='border border-gray-200 rounded w-full p-2 mt-1 outline-tertiary' required />
                    </div>
                )}
                <div className='w-full'>
