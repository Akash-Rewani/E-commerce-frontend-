...
const ProductList = () => {
  ...
  const toggleStock = async (productId, inStock) => {
    try {
      const { data } = await axios.post('/product/stock', { productId, inStock })
      if (data.success) {
        fetchProducts()
          ...
          <div key={product._id} className='grid grid-cols-[1fr_3.5fr_1fr_1fr_1fr] items-center gap-2 p-2 bg-white rounded-lg'>
            ...
            <div>
              <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                ...
                <div className='w-10 h-6 bg-gray-400 rounded-full peer peer-checked:bg-green-400 transition-colors duration-200'></div>
